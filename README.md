...
浮窗的余额有两种取法，代码里是**先试 DSH、失败再直连**：

| 顺序 | 数据源 | 能拿到 | 状态点 |
|---|---|---|---|
| 1️⃣ 优先 | DSH 引擎的 `http://127.0.0.1:3080/dsh-whale/balance.json` | 余额、**今日已用**、**峰谷时段**，与网页挂件完全一致 | 显示绿点 |
| 2️⃣ 回退 | 直连 `https://api.deepseek.com/user/balance` | 只有余额（官方接口不返回用量） | 隐藏 |

- DSH 在线时气泡长这样：`¥ 8.83 · 今日 ¥0.87 空闲`
- DSH 关掉后自动退化成：`¥ 8.83`
- 两种都失败时**沿用上次数字**，不闪错误
- DSH 的超时只有 2.5 秒，所以引擎没开时不会卡着等

联动到什么程度：**数据联动**（读 DSH 插件的接口）。做不到「点鲸鱼打开 DSH 界面」——那需要启动其他 App 的能力，本机 `am` 因身份不匹配被系统拒绝。
